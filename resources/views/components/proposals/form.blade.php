<div>
	{{-- new proposal --}}
	<button class="bg-[#5354FD] text-white font-bold tracking-wide uppercase px-8 py-3 rounded-[4px] hover:bg-[#1f20a6] transition duration-300 ease-in-out" wire:click="$set('modal', true)">
		Enviar uma proposta
	</button>

	{{-- modal with form using "ui.modal" template --}}
	<x-ui.modal>
		<form class="flex flex-col gap-6" wire:submit="save">
			...
			<div class="flex gap-4">
				<div class="w-2/3 gap-2 flex flex-col">
					...
					<input wire:model="email" type="email" class="w-full bg-[#1E1E2C] text-white p-3 focus:outline-none focus:ring-0 border border-[#1E1E2C]" placeholder="Insira o seu e-mail" />
				...
				<div class="w-1/3 gap-2 flex flex-col">
					...
					<div class="flex" x-data="{ hours: @entangle('hours') }">
						<button type="button" class="bg-[#2E2E3C] hover:bg-[#414155] transition duration-300 ease-in-out text-[#C3C3D1] py-2 px-2.5 text-3xl" @click="hours--">-</button>

						<input wire:model="hours" type="number" class="bg-[#1E1E2C] text-white p-0 w-[40px] font-bold focus:outline-none focus:ring-0 text-center border border-[#1E1E2C] focus:ring-blue-500" />

						<button type="button" class="bg-[#2E2E3C] hover:bg-[#414155] transition duration-300 ease-in-out text-[#C3C3D1] py-2 px-2 text-3xl" @click="hours++">+</button>
					</div>
                    @error('hours') <span class="text-red-400 text-sm">{{ $message }}</span> @enderror
				</div>
			</div>
			<div>
                ...
                @error('agree') <span class="text-red-400 text-sm">{{ $message }}</span> @enderror
			</div>
